...
library(tidyr)
library(xtable)
files <- c(list.files(path = "./benchmark_results", # rebuttal results
                      pattern = ".*\\.csv$",
                      all.files = FALSE, full.names = TRUE, recursive = FALSE, ignore.case = TRUE,
                      include.dirs = FALSE, no.. = FALSE),
           list.files(path = "../benchmark_results", # original submission results
                      pattern = ".*\\.csv$",
                      all.files = FALSE, full.names = TRUE, recursive = FALSE, ignore.case = TRUE,
                      include.dirs = FALSE, no.. = FALSE)
           )
...
##### 
# only for first train/test split -> drop results for further train/test splits in CTGAN, TVAE, gen_rf!
#####
reps_5 = df %>% filter(model %in% c("CTGAN", "TVAE", "gen_rf", "oracle")) %>% group_by(model, dataset, classifier) %>%  filter(row_number()==1)
reps_1 = df %>% filter(model %in% c("CTABGAN+", "ITGAN", "RCCGAN"))
df = rbind(reps_5, reps_1)
...
# table to latex
print(xtable(xtable_f1, digits = 4), include.rownames = F,  booktabs = T)
